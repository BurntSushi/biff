...
As a modern suite of tools that follow the Unix philosophy, bttf relies
...
With bttf, this is much more streamlined:
...
datetime parsing. This also of course means you can do things with bttf that
...
$ printf '2025-05-01T00-04\n2025-07-31T00-04\n' \
    | bttf time in system
    | bttf span since -l year -r 2025-11-01
6mo
3mo 1d
```
...
Separately from composition, bttf also supports a lot of things that GNU
date does not. For example, bttf lets you use a `strptime`-like API to parse
...
With bttf, you can just use `strptime` to parse it as needed:

```console
$ bttf time parse -f '%d/%m/%y' 05/09/25 | bttf time fmt -f '%c'
Fri, Sep 5, 2025, 12:00:00 AM EDT
```

bttf also provides first class support for duration strings like
`2 weeks, 5 hours ago`, including rounding durations. And bttf also supports
...
$ bttf tag lines /tmp/access.log | bttf time in system | bttf time fmt -f '%c' | head -n1 | bttf untag -s
Wed, Apr 30, 2025, 1:25:14 AM EDT       INFO    http.log.access.log0    handled request
```

## dateutils

[dateutils] is a suite of distinct programs for datetime handling. Like bttf,
it does a lot more than what GNU date can do. Moreover, like bttf, it enables
...
And here's the bttf equivalent:

```console
$ bttf time seq daily 2010-01-01 -c5 | bttf span until -l year -r 2009-12-01
1mo
1mo 1d
1mo 2d
1mo 3d
...
Overall, bttf and dateutils serve similar use cases. dateutils does seem to
have more of a focus on civil dates, where as bttf is more deeply connected to
instants in a time via deeper integration with time zones. The biggest features
bttf probably has over dateutils are:

* More comprehensive support for generating sequences of datetimes (thanks to
bttf's RFC 5545 implementation).
* Support for tagging arbitrary data into JSON lines with datetimes. All bttf
commands can then accept this data on stdin and transform tagged datetime
values.
* bttf has better handling for rounding time spans.

With that said, there are currently some use cases that are easier with
dateutils. While its "tagging support" isn't quite as sophisticated as bttf's,
...
bttf can do this too, but it's a fair bit more annoying at present:

```console
$ bttf tag lines -e '[0-9]{4}-[0-9]{2}-[0-9]{2}' todo \
    | bttf time parse -f flexible \
    | bttf time add 1mo \
    | bttf time start-of month \
    | bttf time fmt -f '%F' \
    | bttf untag -s
pay cable       2012-03-01
pay gas 2012-03-01
...
involved in finding, parsing and formatting the dates. But bttf does have the
...
bttf does something similar (with a few more bells and whistles) via a
...
$ bttf span since 2025-03-20 | bttf span round -l day -s day
51d
```

bttf's duration rounding is a bit more declarative. You specify the bounds of
units you want, and bttf does the rest. In contrast, with dateutils, it's hard
...
Another thing bttf does for duration rounding is that it knows that not all
